...
def IG(D, index, value):
    ...
    H_Dy = sum([0 if N_Dy1 == 0 else -(N_Dy1 / N_Dy) * math.log2(N_Dy1 / N_Dy),
                0 if N_Dy == N_Dy1 else -((N_Dy - N_Dy1) / N_Dy) * math.log2((N_Dy - N_Dy1) / N_Dy)])
    H_Dn = sum([0 if N_Dn1 == 0 else -(N_Dn1 / N_Dn) * math.log2(N_Dn1 / N_Dn),
                0 if N_Dn == N_Dn1 else -((N_Dn - N_Dn1) / N_Dn) * math.log2((N_Dn - N_Dn1) / N_Dn)])
...
def G(D, index, value):
    ...
    G_Dy, G_Dn = 1 - sum([(N_Dy1 / N_Dy) ** 2, ((N_Dy - N_Dy1) / N_Dy) ** 2]), 1 - sum(
        [(N_Dn1 / N_Dn) ** 2, ((N_Dn - N_Dn1) / N_Dn) ** 2])
...
def CART(D, index, value):
    ...
    CART_Dy_Dn = 2 * (N_Dy * N_Dn / (N ** 2)) * sum(
        [abs(N_Dy1 / N_Dy - N_Dn1 / N_Dn), abs((N_Dy - N_Dy1) / N_Dy - (N_Dn - N_Dn1) / N_Dn)])
...
def classifyIG(train, test):
    ...
    class_less_best_value = 1 if sum(D_conc[D_conc[:, best_index] <= best_value, -1]) >= len(
        D_conc[D_conc[:, best_index] <= best_value, -1]) / 2 else 0
...
def main():
    ...
    print('result of testsets for IG: {}\ntrue label of testsets {}\n============='.format(
        classifyIG(train_file, test_file), test_file[1]))
    print('result of testsets for GINI: {}\ntrue label of testsets {}\n============='.format(
        classifyG(train_file, test_file), test_file[1]))
    print('result of testsets for CART: {}\ntrue label of testsets {}\n============='.format(
        classifyCART(train_file, test_file), test_file[1]))
